...
namespace FORM
{
    public partial class FRM_DELI_STATUS_UPPER : Form
    {
        public FRM_DELI_STATUS_UPPER()
        {
            InitializeComponent();
        ...
        #endregion
        #region DB
        private DataSet SELECT_TMS_DATA(string ARG_PROC_NAME, string ARG_QTYPE, string ARG_DATE, string ARG_PLANT_CD,string ARG_LINE_CD)
        {
            try
            {
        ...
        private async Task BindingData4Chart(string Qtype)
        {
            try
            {
                ...
                string LineCode = ComVar.Var._strValue1;
                string ProcName = string.Empty;
                switch (ComVar.Var._Area)
                {
                    case "2120":
                        ProcName = "PKG_TMS_LONGTHANH.SELECT_LT_UPPER_STATUS_BY_LINE";
                        break;
                    case "2210":
                        ProcName = "PKG_TMS_TANPHU.SELECT_TP_UPPER_STATUS_BY_LINE";
                        break;
                }
                DataSet ds = SELECT_TMS_DATA(ProcName, "", "","2110", LineCode);
            ...
            try
            {
                ...
                switch (ComVar.Var._Area)
                {
                    case "2120":
                        ...
                        break;
                    case "2210":
                        ProcName = "PKG_TMS_TANPHU.SELECT_UPPER_STATUS_BY_LINE";
                        break;
                }
                DataSet ds = SELECT_TMS_DATA(ProcName, "", "", "2110", LineCode);
                dt = ds.Tables[0];
        ...
        private void FRM_DELI_STATUS_V6_VisibleChanged(object sender, EventArgs e)
        {
            if (this.Visible)
            {
                string LineName = ComVar.Var._strValue2;
                string AreaName = ComVar.Var._Area;
                switch (AreaName)
                {
                    case "2120":
                        AreaName = "VJ2(Long Thanh)";
                        break;
                    case "2210":
                        AreaName = "VJ3(Tan Phu)";
                        break;
                }
                lblTitle.Text = string.Format( "Upper {0} —> {1} Delivery Detail", AreaName,LineName);
                lblDate.Text = string.Format(DateTime.Now.ToString("yyyy-MM-dd\nHH:mm:ss")); //Gán dữ liệu giờ cho label ngày giờ
                cCount = 60;
                tmrDate.Start();
            }
